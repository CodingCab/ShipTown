<template>
    <div>
        ...
        <search-and-option-bar :isStickable="true">
            <barcode-input-field placeholder="Search" :url_param_name="'filter[name_contains]'"
                                 @barcodeScanned="loadData(1)"></barcode-input-field>
            <template v-slot:buttons>
                <b-dropdown dropleft no-caret variant="primary" class="ml-2" dusk="new_data_collection"
                            id="new_data_collection">
                    ...
                    <b-dropdown-item @click="createBlankCollection" name="create_blank_collection_button"
                                     dusk="create_blank_collection_button" id="create_blank_collection_button">Blank
                    </b-dropdown-item>
                </b-dropdown>
            </template>
        ...
        <template v-for="record in data">
            <swiping-card :disable-swipe-right="true" :disable-swipe-left="true">
                <template v-slot:content>
                    <div role="button" dusk="data_collection_record" class="row"
                         @click="openDataCollection(record['id'])">
                        ...
                        <div class="col-cols col-sm-12 col-lg-6 bottom text-right">
                            <text-card v-if="record['deleted_at'] !== null"
                                       :label="formatDateTime(record['deleted_at'], 'dddd - MMM D HH:mm')"
                                       text="ARCHIVED" class="float-left text-left"></text-card>
        ...
        <b-modal id="new-collection-modal" no-fade hide-header title="New Data Collection"
                 @ok="createCollectionAndRedirect" @shown="prepareNewCollectionModal">
            <input dusk="collection_name_input" id="collection_name_input" v-model="newCollectionName" type="text"
                   @keyup.enter="createCollectionAndRedirect" class="form-control" placeholder="New Collection name">
            <hr>
            <vue-csv-import
                v-model="csv"
                headers
